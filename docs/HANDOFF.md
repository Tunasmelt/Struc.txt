...
- **Phase:** 5 — Search & filters: built, pending live verification. The user has manually tested Phases 1-5 themselves as of this session (see session log) — that's the first real, live, logged-in confirmation this project has had beyond structural/build verification, and it should supersede this file's earlier "not verified" caveats for whatever Phases 1-5 items they actually exercised. This file wasn't told exactly which specific gate items were covered, so gate checkboxes in `PHASES_AND_GATES.md` for Phases 1-4 haven't been mechanically flipped to `[x]` off the back of it — worth the user (or next session) confirming which items to actually check off.
- **Gate status:** Phase 5's filter combos (tag/date/template) and real Postgres full-text search (via `searchNoteIds`) are built. Found and fixed a real bug while building it: the Phase 0 search trigger referenced a nonexistent `notes.body` column, which as written would error on every note insert — fixed in `006_fix_search_trigger_and_fts.sql`, which also makes `notes.search` actually pick up structured content once restructuring finishes (nothing did that before). **This migration needs to be applied to Supabase before search will work at all.** Two other pending migrations from prior sessions remain: `004_seed_preset_templates.sql` and `005_add_pinned_archived_to_notes.sql`.
