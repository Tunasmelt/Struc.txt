...
- **Phase:** 6 — Audio capture: built, unverified — no real browser+microphone test has been possible in this environment. The user manually tested Phases 1-5 themselves before this session and confirmed to proceed, which is the project's first real live confirmation beyond structural/build verification for that range.
- **Gate status:** Recording (MediaRecorder), live transcript (Web Speech API, feature-detected), private Supabase Storage upload, and an async Groq Whisper cleanup pass that then runs the unchanged Phase 2 restructuring pipeline are all built and `tsc`/`build` clean (`app/actions/audio.ts`, `lib/ai/transcribe.ts`, `components/board/CaptureModal.tsx`'s new "Record" tab, a "▶ Play recording" affordance in `Drawer.tsx`). **None of it has been exercised with a real microphone** — this is a structural/code-review-level pass only. The user confirmed `006_fix_search_trigger_and_fts.sql` is applied and Phases 1-5 are manually verified; `supabase/migrations/007_audio_storage_bucket.sql` (new, this session) still needs to be applied to Supabase before recording will work at all.
